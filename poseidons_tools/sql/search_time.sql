...
search_time AS (
    SELECT search_id, first.auth_user, DATE_PART('Minute', last.pre_handle_date_time - first.pre_handle_date_time) AS minutes_to_search, first.pre_handle_date_time AS search_date FROM (
...
SELECT
    SUM(CASE WHEN search_date >= CURRENT_DATE - INTERVAL '1 year' THEN 1 ELSE 0 END) AS one_year_search_sessions,
    COUNT(DISTINCT search_id) AS all_time_search_sessions,
